...
// Limitation settings
#define I_MOT_MAX       1              // 15 // 3 = stopable by hand [A] Maximum single motor current limit
#define I_DC_MAX        3              // 17 // 5 = stopable by hand[A] Maximum stage2 DC Link current limit for Commutation and Sinusoidal types (This is the final current protection. Above this value, current chopping is applied. To avoid this make sure that I_DC_MAX = I_MOT_MAX + 2A)
#define N_MOT_MAX       50            // 1000 [rpm] Maximum motor speed limit 100 = 3kmh
...
// ############################ VARIANT_USART SETTINGS ############################
  ...
  #define PRI_INPUT2             3, 372, 1090, 1810, 3     // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #ifdef DUAL_INPUTS
...
// ############################ VARIANT_HOVERCAR SETTINGS ############################
  ...
  #define AUX_INPUT1              2, -100, 0, 100, 1  // Sideboard Steer    TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #define AUX_INPUT2              2, -100, 0, 100, 1  // Sideboard Speed    TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
...
// ########################### UART SETIINGS ############################
  ...
  #define SERIAL_TIMEOUT          320                     // 160 [-] Serial timeout duration for the received data. 160 ~= 0.8 sec. Calculation: 0.8 sec / 0.005 sec
#endif
#if defined(FEEDBACK_SERIAL_USART2) || defined(CONTROL_SERIAL_USART2) || defined(DEBUG_SERIAL_USART2) || defined(SIDEBOARD_SERIAL_USART2)
  #ifndef USART2_BAUD
